...
function saveTasks(){
 ...
 function addTask() {
     const task = taskInput.value.trim(); //Pega o texto e remove os espaços em branco
 ...
 function removeTask(index) {
    ...
    // Cria a mensagem de corfimação, incluindo um emoji de alerta.
    const confirmationMessage = '🤔 Tem certeza que deseja remover a tarefa: "${taskText}"? ';

    //Exibe a caixa de diálogo de corfirmção
    //O código dentro do 'if' só será executado se o usuário clicar em "OK".
    if (confirm(confirmationMessage)) {
        // Se o usuário conrfirmou (clicou em "Ok"):

        // Remove 1 item a pasrtir do 'index' do array.
        tasks.splice(index, 1);

        // Salva o novo estado do array no localStorage.
        saveTasks();

        // Redesenha a lista na tela para reletir a remoção.
        renderTasks();
    }
    // Se o usuário clicar em "Cancelar", nada acontece e a dunção termina.
}
